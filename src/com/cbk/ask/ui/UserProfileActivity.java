...
public class UserProfileActivity extends BaseActivity implements OnClickListener{
	...
	public void asyncFetchUserInfo(String username){
		AskHelper.getInstance().getUserProfileManager().asyncGetUserInfo(username, new EMValueCallBack<EaseUser>() {
			
			@Override
			public void onSuccess(EaseUser user) {
				if (user != null) {
				    AskHelper.getInstance().saveContact(user);
				    if(isFinishing()){
				        return;
				    }
	...
	private void updateRemoteNick(final String nickName) {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				boolean updatenick = AskHelper.getInstance().getUserProfileManager().updateCurrentUserNickName(nickName);
	...
	private void uploadUserAvatar(final byte[] data) {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				final String avatarUrl = AskHelper.getInstance().getUserProfileManager().uploadUserAvatar(data);
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						dialog.dismiss();
