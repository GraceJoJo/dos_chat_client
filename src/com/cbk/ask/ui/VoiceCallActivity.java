...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		AskHelper.getInstance().isVoiceCalling = true;
		callType = 0;
		...
		public void onCmdMessageReceived(List<EMMessage> messages) {
			for (EMMessage message : messages) {
				...
				if (action.equals("prescription")) {//收到处方
					...
					//获取文件名
					try {
						final String str = message.getStringAttribute("prescription");
						//弹出提示框
						runOnUiThread(new Runnable() {
							public void run() {
								Intent intent = new Intent();
								intent.setClass(VoiceCallActivity.this, WebViewActivity.class);
...
//                                        callStateTextView.setText(st7);
    ...
    protected void onDestroy() {
        super.onDestroy();
        AskHelper.getInstance().isVoiceCalling = false;
    }
